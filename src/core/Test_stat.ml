...
let to_printbox_l ?to_link l : PB.t =
  ...
  let r1 =
    [mk_row "sat" get_sat @@ pb_int_color PB.Style.(fg_color Green);
     ...
     mk_row1 "valid_proof" get_valid_proof @@ pb_int_color PB.Style.(fg_color Green);
     mk_row1 "invalid_proof" get_invalid_proof @@ pb_int_color PB.Style.(fg_color Red);
...
let of_db_for ~(prover:Prover.name) (db:Db.t) : t =
  ...
  { sat; unsat; timeout; memory; unknown;
    valid_proof; invalid_proof;
    errors; custom; total; total_time; }
...
let pp out (s:t) : unit =
  Fmt.fprintf out
    "(@[<hv>:unsat %d@ :sat %d@ :solved %d@ :errors %d@ :unknown %d@ \
     :valid-proof %d@ :invalid-proof %d@ :timeout %d@ \
     :total %d@ :total_time %.2f@])"
    s.unsat s.sat (s.sat + s.unsat) s.errors s.unknown
    s.valid_proof s.invalid_proof s.timeout
