...
let run ~limits ~proof_file prover pb : _ Run_result.t =
  Error.guard
    (Error.wrapf "run prover '%s' on problem '%s' (proof file %a)"
  ...
  in
  Log.info (fun k ->
      k "running %-15s/%-30s (timeout %a)..." prover.Prover.name pb.Problem.name
        Limit.Time.pp timeout);
  ...
  let result = Run_result.make_from_prover prover ~timeout pb raw in
  Logs.debug (fun k ->
      let open Run_proc_result in
      k "output for %s/%s: `%s`, `%s`, errcode %d" prover.Prover.binary
        pb.Problem.name raw.stdout raw.stderr raw.errcode);
  result
...
let run_proof_check ~limits prover checker pb ~proof_file : check_res =
  ...
  in
  let res =
    Run_result.make_from_checker prover checker ~timeout:Limit.Time.default pb
      raw
  in
  res
...
let pp_result ~w_prover ~w_pb out (res : Test.result) : unit =
  ...
  Logs.info (fun k ->
      k "result for `%s` with %s: %s (%.1fs, expected %s)" prover_name pb_name
        (Res.to_string res.res) res.raw.rtime
        (Res.to_string res.problem.Problem.expected));
  Fmt.fprintf out "%-*s%-*s : %a (%.2fs)@." w_prover
    (Misc.truncate_right w_prover prover_name)
    w_pb
    (Misc.truncate_left w_pb pb_name)
    pp_res () res.raw.rtime;
  ()

let pp_result_progress ~w_prover ~w_pb r : unit =
  let msg =
    Format.asprintf "%s%a" Misc.reset_line (pp_result ~w_prover ~w_pb) r
  in
  Misc.synchronized_sync (fun () -> Printf.printf "%s%!" msg);
  ()
...
let pp_check_result ~w_prover ~w_pb out (res : check_res) : unit =
  ...
  let pb_name = res.problem.Problem.name in
  Logs.info (fun k ->
      k "checking proof of `%s` with `%s` on %s: %s (%.1fs)" prover_name checker
        pb_name
        (Proof_check_res.to_string res.res)
        res.raw.rtime);
  Fmt.fprintf out "[chk]%-*s%-*s%-*s : %a (%.1fs)@." w_prover
    (Misc.truncate_right w_prover prover_name)
    w_prover
    (Misc.truncate_right w_prover checker)
    w_pb
    (Misc.truncate_left w_pb pb_name)
    pp_res () res.raw.rtime;
  ()
...
let pp_check_result_progress ~w_prover ~w_pb r : unit =
  let msg =
    Format.asprintf "%s%a" Misc.reset_line (pp_check_result ~w_prover ~w_pb) r
  in
  Misc.synchronized_sync (fun () -> Printf.printf "%s%!" msg);
  ()
