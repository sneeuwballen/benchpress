...
let load_config_files (files : string list) : Definitions.t =
  let builtin =
    Yaml_config.load_yaml_string ~previous:Definitions.empty
      Static_data.builtin_config ~cur_dir:(Sys.getcwd ())
  in
  List.fold_left
    (fun defs file -> Yaml_config.load_file ~previous:defs file)
    builtin files
...
let get_definitions () : Definitions.t =
  let conf_files =
    let default_conf = Misc.default_config () in
    (* always add default config file if it exists *)
    if Sys.file_exists (Xdg.interpolate_home default_conf) then
      [ default_conf ]
    else
      []
  in
...
let check_res_an ?(no_failure = false) notify a : unit =
  if List.for_all (fun (_, r) -> Test_analyze.is_ok r) a then
...
let check_compact_res ?(no_failure = false) notify
    (results : Test_compact_result.t) : unit =
...
let list_entries ?(off = 0) ?(limit = max_int) data_dir :
    _ list * [ `Done | `More ] =
