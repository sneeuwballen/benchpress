...
let to_compact_result (self:t) : Test_compact_result.t =
  ...
  {Test_compact_result.cr_analyze; cr_meta=self.meta; cr_stat; cr_comparison; }
        ...
        (fun file ->
           let tr_res =
             Db.exec db
               ...
               file
             |> Misc.unwrap_db (fun() -> spf "listing results for file '%s'" file)
...
let db_to_printbox_table ?offset ?page_size ?link_pb
    ?link_res ?filter_pb ?filter_res db : _ =
