      ...
      List.map
        (fun (file_path, size) ->
          ...
          let meta =
            try
              let _file, cr =
                Bin_utils.load_file_summary ~full:false basename
              in
              Some cr.cr_meta
            with _ -> None
          in
          ...
          let n_res =
            match meta with
            | Some m -> Int32.of_int m.n_results
            | None -> 0l
          in
          let n_b =
            match meta with
            | Some m -> Int32.of_int m.n_bad
            | None -> 0l
          in
          let provers =
            match meta with
      ...
      let provers =
        List.map
          (fun (p_name, stat) ->
            ...
            T.make_prover_stat ~prover:p_name ~sat:(Int32.of_int stat.sat.n)
              ...
              ~total:(Int32.of_int stat.total) ~total_time:stat.total_time
...
let tool_get_job_results ~data_dir : Mcp.tool_def =
  ...
  let handler (args : Yojson.Basic.t) : Yojson.Basic.t =
    ...
    if file = "" then
      ...
      Bin_utils.with_file_as_db ~map_err:(Error.wrapf "using DB '%s'" file)
        (resolve_file ~data_dir file) (fun db ->
...
let tool_query_job_results ~data_dir : Mcp.tool_def =
              ...
              ( "prover",
                `Assoc
                  [
                    "type", `String "string";
                    ( "description",
                      `String "Filter by prover name (partial match)" );
                  ] );
              ( "problem",
                `Assoc
                  [
                    "type", `String "string";
                    ( "description",
                      `String "Filter by problem file (partial match)" );
                  ] );
              ( "result",
                `Assoc
                  [
                    "type", `String "string";
                    ( "description",
                      `String "Filter: sat, unsat, error, timeout, unknown" );
                  ] );
              ( "page",
                `Assoc
                  [
            ...
            List.map
              (fun (k : key) ->
                T.make_query_item ~prover:k.prover ~problem:k.file
                  ...
                  ~expected:(Res.to_string k.file_expect)
                  ~time_s:k.rtime ())
              keys
          in
  ...
  in
  {
    ...
    description =
      "Paginated, filterable query over individual benchmark results.";
    input_schema;
    handler;
  }
