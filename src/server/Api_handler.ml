(** API handler: registers Twirp service handlers with tiny_httpd.
   We copy and extend twirp_tiny_httpd's handler/add_service so that [f]
   receives the raw HTTP request (and thus req.meta, where auth stores the
   user_id). *)
...
let return_error (code : Error_codes.t) msg : H.Response.t =
  ...
  let body =
    ...
    |> Twirp_error.encode_json_error
    |> Yojson.Basic.to_string
  in
  H.Response.make_raw ~headers:[ "content-type", "application/json" ]
    ~code:http_code body
...
let handle_rpc (Handler { rpc; f }) (http_req : string H.Request.t) :
  ...
  try
    ...
    let proto_req =
      ...
      | `JSON ->
        (try rpc.decode_json_req (Yojson.Basic.from_string http_req.H.Request.body)
    ...
    (match content_type with
    | `JSON ->
      H.Response.make_string @@ Ok (Yojson.Basic.to_string @@ rpc.encode_json_res res)
    | `BINARY ->
...
let handle_new_job state http_req (req : Api.new_job_request) =
  ...
  if req.Api.provers = [] then fail Error_codes.Invalid_argument ~msg:"provers is empty";
  if req.Api.paths = [] then fail Error_codes.Invalid_argument ~msg:"paths is empty";
  let timeout_s =
  ...
  let task =
    Task.{ name; synopsis = None; action = Action.Act_run_provers action;
           defined_in = None }
  in
  let job_id = Task_queue.push state.task_q task in
...
let handle_get_job_status state http_req (req : Api.get_job_status_request) =
  ...
  | Some uid when uid <> user_id -> fail Error_codes.Not_found ~msg:"job not found"
  | Some _ -> ());
  ...
  | Task_queue.Unknown ->
    if Auth.get_job_cancelled state.auth ~job_id then
      ...
      Auth.set_job_completed state.auth ~job_id ~result_file:"";
...
let register ~auth ~task_q ~defs ~data_dir ~http_server =
  ...
  add_service ~middlewares:[ Auth_middleware.middleware auth ] http_server service
