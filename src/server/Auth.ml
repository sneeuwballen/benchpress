...
let list_api_keys (self : t) ~(user_id : string) : (string * string) list =
  ...
  Fun.protect ~finally:(fun () -> Mutex.unlock self.mu) @@ fun () ->
  Db.exec self.db
    {|SELECT key, created_at FROM api_key WHERE user_id=?|}
...
let authenticate (self : t) ~(key : string) : string option =
  ...
  Fun.protect ~finally:(fun () -> Mutex.unlock self.mu) @@ fun () ->
  Db.exec self.db
    {|SELECT user_id FROM api_key WHERE key=?|}
    ~ty:Db.Ty.(p1 text, p1 text, (fun s -> s))
    key
    ~f:Db.Cursor.next
...
let get_job_user (self : t) ~(job_id : string) : string option =
  ...
  Fun.protect ~finally:(fun () -> Mutex.unlock self.mu) @@ fun () ->
  Db.exec self.db
    {|SELECT user_id FROM job WHERE id=?|}
    ~ty:Db.Ty.(p1 text, p1 text, (fun s -> s))
    job_id
    ~f:Db.Cursor.next
...
let set_job_cancelled (self : t) ~(job_id : string) : unit =
  ...
  Fun.protect ~finally:(fun () -> Mutex.unlock self.mu) @@ fun () ->
  Db.exec_no_cursor self.db
    {|UPDATE job SET cancelled=1 WHERE id=?|}
    ~ty:Db.Ty.[text]
    job_id
  |> Misc.unwrap_db (fun () -> "auth.set_job_cancelled")

let set_job_completed (self : t) ~(job_id : string) ~(result_file : string) : unit =
  Mutex.lock self.mu;
  Fun.protect ~finally:(fun () -> Mutex.unlock self.mu) @@ fun () ->
  Db.exec_no_cursor self.db
    {|UPDATE job SET completed=1, result_file=? WHERE id=?|}
    ~ty:Db.Ty.[text; text]
    result_file job_id
...
let get_job_cancelled (self : t) ~(job_id : string) : bool =
  ...
  let r =
    Db.exec self.db
      {|SELECT cancelled FROM job WHERE id=?|}
